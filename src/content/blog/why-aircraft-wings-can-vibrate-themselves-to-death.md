...
Put a concentrated mass partway out along a wing and you change its natural frequencies in both plunge and pitch — and the relationship between those frequencies is a big part of what governs when their phases couple dangerously. Configurations that would be perfectly safe on a conventional wing can have flutter behavior that has to be re-examined from scratch. The clean two-degree-of-freedom model — plunge and pitch, two coupled equations — becomes a much larger, messier system when there are six motors distributed along the span, each adding its own inertia and shifting the shear center of each wing section.
